...
pub struct SandboxEntry {
    ...
    /// Serializes process creation against teardown.
...
impl Drop for SandboxEntry {
    fn drop(&mut self) {
        // A request may still hold this entry after removal from the registry.
        // Its pre-exec hook must never see a closed (possibly reused) descriptor.
        if let Confinement::Landlock(fd) = self.confinement {
...
impl SandboxRegistry {
    ...
    pub fn delete(&self, id: &str) -> Option<Result<(), String>> {
        ...
        *entry.deleted.lock().unwrap() = true;
