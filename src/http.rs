...
pub(crate) fn parse_head(head: &[u8]) -> io::Result<Request> {
    ...
    if !url.starts_with('/') || url.bytes().any(|b| b <= 0x20 || b == 0x7f) {
    ...
    for line in lines {
        ...
        if value.bytes().any(|b| (b < 0x20 && b != b'\t') || b == 0x7f) {
            return Err(bad("illegal control character in header value"));
        }
...
mod tests {
    ...
    fn ambiguous_framing_is_refused() {
        let cases = [
            ("X-Test: a\nb", "bare newline in value"),
            ("X-Test: a\rb", "bare carriage return in value"),
            ("X-Test: \0", "NUL in value"),
    ...
    #[test]
    fn control_characters_in_targets_are_refused() {
        for target in ["/a\nb", "/a\rb", "/a\0b", "/a\tb", "/a\x7fb"] {
            assert!(parse(&format!("GET {target} HTTP/1.1\r\nHost: x\r\n\r\n")).is_err());
        }
    }
